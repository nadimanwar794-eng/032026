                ...
                return (
                    <div className="space-y-4">
                        ...
                        <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-4 space-y-3">
                            <div>
                                <p className="text-sm font-black text-amber-900 flex items-center gap-2">🏆 Level Score Thresholds</p>
                                <p className="text-[10px] text-amber-700 mt-0.5">Har level ke liye minimum score set karo. Khali chhodo = default system value use hogi.</p>
                            </div>
                            <div className="grid grid-cols-1 gap-2 max-h-80 overflow-y-auto pr-1">
                                {[
                                    ...
                                    const cur = (localSettings.levelScoreOverride as Record<string,number> | undefined)?.[String(level)];
                                    return (
                                        <div key={level} className="flex items-center gap-3 bg-white rounded-xl px-3 py-2 border border-amber-100">
                                            ...
                                            <div className="w-24 shrink-0">
                                                <p className="text-[11px] font-bold text-slate-700">L{level} {label}</p>
                                                <p className="text-[9px] text-slate-400">Default: {def.toLocaleString()}</p>
                                            </div>
                                            <input
                                                type="number" min={0}
                                                value={cur ?? ''}
                                                ...
                                                onChange={e => {
                                                    ...
                                                    const overrides = {...((localSettings.levelScoreOverride as Record<string,number> | undefined) || {})};
                                                    if (val === '') { delete overrides[String(level)]; } else { overrides[String(level)] = Number(val); }
                                                    updateSetting('levelScoreOverride' as any, overrides);
                                                }}
                            ...
                            <button
                                onClick={() => updateSetting('levelScoreOverride' as any, {})}
                                className="w-full py-2 rounded-xl border border-amber-200 text-amber-700 text-xs font-bold hover:bg-amber-100 transition-all"
                            >↩ Reset All Levels to Default</button>
                        </div>
                    </div>
                );
            })()}
