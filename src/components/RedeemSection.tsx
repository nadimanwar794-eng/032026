  ...
  const handleRedeem = async () => {
    ...
    try {
        ...
        if (targetCode.type === 'SUBSCRIPTION') {
            ...
            // Duration in ms for this plan
            const TIER_DURATION_MS: Record<string, number> = {
                WEEKLY:     7   * 24 * 3600000,
                MONTHLY:    30  * 24 * 3600000,
                '3_MONTHLY': 90  * 24 * 3600000,
                YEARLY:     365 * 24 * 3600000,
                LIFETIME:   365 * 10 * 24 * 3600000,
            };
            const durationMs = TIER_DURATION_MS[subTier] ?? (30 * 24 * 3600000);

            // ── STACKING & QUEUE LOGIC ──────────────────────────────
            const existingEndMs = updatedUser.subscriptionEndDate
                ? new Date(updatedUser.subscriptionEndDate).getTime()
                : 0;
            const hasActiveSubscription = existingEndMs > now.getTime();

            let subStartDate = now; // default: start now
            let isQueued = false;

            if (hasActiveSubscription) {
                if (updatedUser.subscriptionLevel === subLevel) {
                    // SAME PLAN → stack on top of existing end date
                    subStartDate = new Date(existingEndMs);
                } else {
                    // DIFFERENT PLAN → queue after current expires
                    subStartDate = new Date(existingEndMs);
                    isQueued = true;
                }
            }

            let endDate: Date | null = subTier === 'LIFETIME'
                ? null // Lifetime = no end, handled separately
                : new Date(subStartDate.getTime() + durationMs);
            // Lifetime: 10 yr from start
            if (subTier === 'LIFETIME') endDate = new Date(subStartDate.getTime() + durationMs);
            ...
            // For queued plan: store pending info, don't override current active plan fields
            if (isQueued) {
                (updatedUser as any).pendingMembership = {
                    tier: subTier,
                    level: subLevel,
                    activatesAt: subStartDate.toISOString(),
                    endDate: isoEndDate,
                };
                // Keep current subscription fields unchanged (don't override subscriptionLevel/EndDate)
            } else {
                // STACKING or fresh: update legacy fields
            ...
            // Always mark as premium if they have any active or stacked plan
            updatedUser.isPremium = true;
